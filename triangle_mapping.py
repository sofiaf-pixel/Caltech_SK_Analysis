...
Ndet_mce_dark= 2*4
Ndet_det = 2*33
npixel = 25 # pixels in one tile
npixel_dark = 4
ntile = 1
lld = 5 # how many dets in one row pyhsically
...
def prepare_data_dark(input_inmce, itile=0, mapping="BA"):
	...
	for idet in range(Ndet_mce_dark):
		...
		if POLAR_dark[idet] == 'DA':
			...
			idet_col = int(IDet_col_dark[idet])
			idet_row = int(IDet_row_dark[idet])
			data_arr[(idet_col-1)*lld_dark+(idet_row-1)+npixel] = DATA[idet]
...
def plot_a_map(data_array,triangles,x,y,mask_wire,itile,value_min,value_max,plot_title,plot_unit,inpath, ifshow=False, cmap="jet"):
	...
	fig.savefig(inpath+plot_title+'_Tile%d.png'%itile)
...
def plot_a_map_dark(data_array,triangles,x,y,mask_wire,itile,value_min,value_max,plot_title,plot_unit,inpath, ifshow=False, cmap="jet"):
        ...
        plt.text(1.5, 2.9, 'mce row 0', fontsize=10)
        plt.text(2.5, 2.9, 'mce row 1', fontsize=10)
